...
def show_mean_charts(single, mean_df, total_energy):
    ...
    tab_line.line_chart(mean_df, x=TIME, y=POWER, use_container_width=True)
    tab_area.area_chart(mean_df, x=TIME, y=POWER, use_container_width=True)
    tab_bar.bar_chart(mean_df, x=TIME, y=POWER, use_container_width=True)


def show_errorband_charts(single, mean_df, power_df):
    # Only show these if not just a single file
    if not single:
        # Get a time indexed mean df and calculate the std and conf data across the DataFrames
        mean_tip = mean_df.set_index(TIME)[POWER]
        std_data = power_df.std(axis=1)
        conf_data = std_data * 2

        # Combine the data into one time, mean, std/conf DataFrame
        std_df = pd.concat([mean_tip, std_data], axis=1, keys=["MEAN", "STD"]).reset_index()
        conf_df = pd.concat([mean_tip, conf_data], axis=1, keys=["MEAN", "CONF"]).reset_index()
...
def main():
    ...
    if uploaded_files:
        ...
        # Show the data analysis charts (single handled internally to easily keep the charts order)
